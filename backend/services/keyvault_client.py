...
class KeyVaultClient:
    """
    Azure Key Vault client for fetching secrets.
    ...
    def get_secret(self, secret_name: str) -> str:
        """
        Retrieve a secret value from Key Vault by name.
        Raises if Key Vault URL is not configured (returns empty string for local dev).
        """
        if not settings.key_vault_url:
            logger.warning(
                f"KEY_VAULT_URL not set; cannot fetch secret '{secret_name}' from Key Vault"
            )
            return ""
        try:
